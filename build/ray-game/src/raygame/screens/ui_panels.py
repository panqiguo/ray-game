...
def draw_profile_modal(font: Font | None, state: GameState, growth_defs=GROWTH_DEFS) -> None:
    ...
    draw_text(font, f"成长点数 {state.growth_points}", int(rect.x) + 20, header_y, 18, Color(212, 196, 132, 255))
    draw_text(font, f"已拥有 {len(state.unlocked_growths)} 项成长", int(rect.x) + 175, header_y, 18, LIGHTGRAY)
    ...
    draw_text(font, "可选成长", int(left.x) + 12, int(left.y) + 10, 18, Color(212, 196, 132, 255))
    draw_text(font, "已拥有成长", int(right.x) + 12, int(right.y) + 10, 18, Color(212, 196, 132, 255))
    ...
    if state.pending_growth_choices:
        for growth_id in state.pending_growth_choices:
            ...
            draw_text(font, growth.title, int(card_rect.x) + 12, int(card_rect.y) + 10, 18, RAYWHITE if can_claim else Color(118, 118, 118, 255))
            body_lines = wrap_text_lines_any(font, growth.description, card_rect.width - 24, 15)
            body_y = int(card_rect.y) + 34
            for line in body_lines[:2]:
                draw_text(font, line, int(card_rect.x) + 12, body_y, 15, LIGHTGRAY)
                body_y += 16
            button_label = "解锁" if can_claim else "点数不足"
            if text_button(font, Rectangle(card_rect.x + card_rect.width - 92, card_rect.y + card_rect.height - 28, 80, 20), button_label, 14, disabled=not can_claim):
                claim_growth(state, growth_id)
                end_layer("profile")
                return
            choice_y += 102
    else:
        draw_text(font, "当前没有新的成长可选。", int(left.x) + 12, choice_y + 6, 17, Color(140, 140, 140, 255))

    owned_y = int(right.y) + 40
    if state.unlocked_growths:
        for growth_id in sorted(state.unlocked_growths):
            growth = growth_defs[growth_id]
            draw_text(font, f"· {growth.title}", int(right.x) + 12, owned_y, 17, LIGHTGRAY)
            owned_y += 24
    else:
        draw_text(font, "暂无已拥有成长。", int(right.x) + 12, owned_y + 6, 17, Color(140, 140, 140, 255))
    end_layer("profile")
