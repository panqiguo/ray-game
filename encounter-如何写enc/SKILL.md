...
1. `store` 是“名字绑定值”：`(progress (clock ...))`、`(phase 'yard)`。
2. 裸 symbol 先 lookup；quoted symbol 才是字面量。
3. `view / children / actions / show-clocks` 里写的是表达式，不是专用小语法。
4. `scene / action / reacts / effect` 在组织 IR；`if / when / cond` 在做选择。
...
2. 提取事实：把需要持久化的事实放进 `store`。持续变化的量写成 `(name (clock ...))`，其他阶段/路径/真假直接写成 `(name value)`。
...
- 持续推进量：`(name (clock ...))`
- 一次性真假事实：`(name true/false)`
- 阶段/历史记忆：`(name 'phase_name)`
- 自然推进/自动收束：`reacts`
- 当前画面和可做的事：`scene`
- 玩家方法：统一写成 `action`
...
- `store` 现在是“名字绑定值”，不是旧的 `clock/value/flag` 三分写法
- 裸 symbol 先 lookup；quoted symbol 才是字面量
- clock 当前值必须用 `(clock-value ...)`
- 通用风险/花色/结局常量优先从 `(include "core_symbols.enc")` 获得；本地阶段/路径/状态通常直接写 quoted symbol，例如 `'entry`
...
如果只需要一页规则，优先看 `language.md` 开头的“语言心智”部分，不必先通读全文。
