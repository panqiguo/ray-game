  ...
  (node
    ...
    :actions (list
      ...
      (action
        ...
        :effects (list (effect 'start-encounter '潜入别墅))))))
  ...
  (node
    ...
    :actions (list
      ...
      (when
        ...
        (action
          :title "问老板有没有别的交易"
          :desc "老板吐出一口雪茄烟圈，说除了别墅那单，他手里还有个需要收拾人的小活。"
          :effects (list
            (effect 'set villa_job_taken true)
            (effect 'set thug_job_unlocked true)
            (effect 'start-quick-dialogue "# 黑车厂的额外生意\n\n黑车厂老板把雪茄屁股碾灭在满是油污的地板上，说城南有个替高利贷放风的小混混最近越来越不知死活。有人愿意出钱，想让你去给那小子一点能记住几周的教训。\n\n他又补了一句，说如果你还嫌来钱太慢，山顶那边也有一栋别墅正等着人去翻窗。干完哪单，你都能离那辆没挂牌的福特更近一点。"))))
      )))

(define thug_job_spot
  (node
    :title "放债人的后巷地址"
    :desc "黑车厂老板随手把地址写在油污账单背面。巷子不长，事情也不会太复杂。你只需要去那里，把一个嚣张过头的小混混打得重新学会看人脸色。"
    :position '(930 332)
    :actions (list
      (action
        :title "开始侦探行动：教训小混混"
        :desc "按着老板给的地址过去，把这桩脏活办干净。"
        :effects (list (effect 'start-encounter '教训小混混))))))
...
(define world-state
  (state
   ...
   (thug_job_unlocked false)))

(define world-reacts
  (reacts
   (react-once-until 被追杀已触发
                     (effect 'start-encounter '逃离疗养院))
   (react
...
(content
 :meta (meta :key 'city_1 :title "洛杉矶街头" :desc "马洛逃离疗养院后的第一段亡命旅程。")
 :state world-state
 :reacts world-reacts
 :root (node
        :title "洛杉矶街头"
        ...
        :children (list
                   ...
                   (when thug_job_unlocked thug_job_spot)
                   )))
