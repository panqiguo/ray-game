...
from sincity.game.conditions import action_is_visible
from sincity.game.queries import current_world_snapshot

from sincity.presentation.table_presenters import (
    PresentedActionCard,
...
from sincity.presentation.tags import location_status_labels
from sincity.presentation.card_model import WORLD_CARD, TableCardModel
...
def present_world_objects(state: GameState) -> tuple[PresentedWorldObject, ...]:
    ...
    for action_id in snapshot.actions_by_location[snapshot.root_location_id]:
...
def current_world_titles(state: GameState) -> tuple[str, str, str]:
    snapshot = current_world_snapshot(state)
    root = snapshot.root_location
    return snapshot.title, root.title, root.description
