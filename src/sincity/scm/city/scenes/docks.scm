...
(define (码头酒馆)
  (node
    ...
    :actions (list
      ...
      (when (and (or ryan_tavern_challenge_unlocked (>= (clock-value ryan_lead) 3))
                 (not ryan_address_found))
        (action
      ...
      (when (and ryan_address_found (not blonde_intro_seen))
        (action
          :title "和金发女郎交谈"
      ...
      (when (and ryan_address_found blonde_intro_seen (not blonde_drinks_done))
        (action
          :title "和金发女郎交谈"
          :desc "她把你的话当作吧台上的水渍一样擦过去，只丢下一句：“再喝点，侦探。清醒的人都太无聊。”"
          :effects (list
            (effect 'set blonde_cold_chatted true))))
      (when (and ryan_address_found blonde_drinks_done (not blonde_flirted))
        (action
          :title "和金发女郎交谈"
      ...
      (when (and ryan_address_found blonde_flirted (not blonde_customer_chased))
        (action
          :title "和金发女郎交谈"
          :desc "她的眼神越过你肩膀，落到那个占便宜的客人身上。她没有开口求你，只是把那一眼停得比平时久。"
          :effects (list (effect 'start-encounter '酒吧赶客人))))
      (when (and ryan_address_found blonde_customer_chased (not blonde_secret_shared))
        (action
          :title "和金发女郎交谈"
      ...
      (when (and ryan_address_found blonde_secret_shared (not blonde_mary_shared))
        (action
          :title "和金发女郎交谈"
...
(define (旧码头)
  (node
    ...
    :actions (list
      (when (not (or ryan_tavern_challenge_unlocked (>= (clock-value ryan_lead) 3)))
        (action
          :title "和码头工人搭话"
      ...
      (when (and (not (or ryan_tavern_challenge_unlocked (>= (clock-value ryan_lead) 3)))
                 (not ryan_notice_checked))
        (action
          :title "查看码头旧公告"
