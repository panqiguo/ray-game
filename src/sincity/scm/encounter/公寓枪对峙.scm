    ...
    :actions (list
      ...
      (action
        ...
        :check (check
          :suits (list 逻辑)
          :risk 'mid
          :ok (outcome "枪口没有放下，但她开始听你说话。" (list (effect 'clock+ truth 2) (effect 'clock+ trust 1)))
          :partial (outcome "她听见了，也更紧张了。" (list (effect 'clock+ truth 1) (effect 'clock+ danger 1)))
          :fail (outcome "她以为你在替弗雷德里克拖时间。" (list (effect 'clock+ danger 1)))))
      (action
        :title "拿出鉴定结果"
        ...
        :check (check
          :suits (list 逻辑)
          :risk 'low
          :ok (outcome "她认出了底片，也认出了自己没法继续躲。" (list (effect 'clock+ truth 2) (effect 'clock+ trust 1)))
          :partial (outcome "她承认底片是真的，但仍然不肯完全相信你。" (list (effect 'clock+ truth 1)))
          :fail (outcome "你递得太快，她的手指也收得更紧。" (list (effect 'clock+ danger 1)))))
      (action
        :title "观察她的持枪手"
        ...
        :check (check
          :suits (list 感知)
          :risk 'mid
          :ok (outcome "她的手在抖。不是杀人的抖，是撑太久的抖。" (list (effect 'clock+ trust 1) (effect 'clock- danger 1)))
          :partial (outcome "你看见一点破绽，也看见她快到极限。" (list (effect 'clock+ trust 1) (effect 'clock+ danger 1)))
          :fail (outcome "你的视线让她误会成动作。" (list (effect 'clock+ danger 1)))))
      (when (clock-at-least-half? trust)
        (action
          ...
          :check (check
            :suits (list 意志 感知)
            :risk 'low
            :ok (outcome "她终于说出了弗雷德里克真正藏起来的东西。" (list (effect 'clock+ truth 2)))
            :partial (outcome "她讲了一半，另一半还卡在喉咙里。" (list (effect 'clock+ truth 1)))
            :fail (outcome "沉默拉得太久，门外传来脚步声。" (list (effect 'clock+ danger 1)))))))))
