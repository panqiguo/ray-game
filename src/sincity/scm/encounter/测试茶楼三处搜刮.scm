...
(content
  ...
  :vars (append
    ...
    (var 'hall_distract (clock :title "大厅干扰" :initial 0 :max 2))
    (var 'warehouse_distract (clock :title "仓库干扰" :initial 0 :max 2))
    (var 'yard_distract (clock :title "后院干扰" :initial 0 :max 2))
    (var 'hall_distracted false)
    (var 'warehouse_distracted false)
    (var 'yard_distracted false)
    (var 'alert (clock :title "警觉" :desc "全局察觉程度。满 6 即失败。" :initial 0 :max 6))
    (var 'completion (clock :title "完成进度" :desc "三个目标各完成一次，总计 3 格。" :initial 0 :max 3))
    (var 'hall_target (clock :title "大厅目标" :initial 0 :max 4))
    (var 'warehouse_target (clock :title "仓库目标" :initial 0 :max 4))
    (var 'yard_target (clock :title "后院目标" :initial 0 :max 5))
    (var 'hall_done false)
