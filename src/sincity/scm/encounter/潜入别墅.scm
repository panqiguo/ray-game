...
(define climb_window
  (action
    ...
    :fail (outcome "你没抓稳，差点摔下去。" (list (effect 'add health -1) (effect 'clock+ alert 1)))
  )
  ))
...
(define left_fight_guard
  (action
    ...
    :fail (outcome "你没压住他。" (list (effect 'add health -1) (effect 'clock+ alert 2)))
  )
  ))
...
(define left_berserk_rush
  (action
    ...
    :fail (outcome "你刚冲出去就被堵了回来。" (list (effect 'set route 'left) (effect 'set patrol 2) (effect 'add health -1) (effect 'clock+ alert 2)))
  )
  ))
...
(define bedroom_act
  (lambda () (if (= entry_method 'front) (bedroom_front_scene) (bedroom_window_scene))))
