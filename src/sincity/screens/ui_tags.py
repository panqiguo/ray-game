...
from sincity.game.queries import get_clock_spec_for_state, get_clock_value
...
def draw_clock_badges(
    ...
    for clock_id in visible_clock_ids:
        ...
        chip_width = max(
            112.0 * scale,
            measure_text_width(font, spec.title, text_size)
            + spec.segments * 18.0 * scale
            + 42.0 * scale,
        )
        chip_data.append((spec, clock_id, chip_width, text_style))

    total_width = sum(c[2] for c in chip_data) + (len(chip_data) - 1) * 10.0 * scale
    chip_height = (18.0 if outside else 20.0) * scale
    vertical = total_width > rect.width

    if vertical:
        x = rect.x + 8.0 * scale if align == "left" else rect.x + rect.width - max(c[2] for c in chip_data) - 8.0 * scale
    else:
    ...
    for spec, clock_id, chip_width, text_style in chip_data:
        if vertical:
            x = rect.x + 8.0 * scale if align == "left" else rect.x + rect.width - chip_width - 8.0 * scale
        chip = Rectangle(x, y, chip_width, chip_height)
        draw_frame(chip, Color(18, 20, 26, 255), Color(90, 94, 100, 220))
        draw_text(
            font, spec.title,
            int(chip.x) + int(8.0 * scale),
            int(chip.y) + (1 if outside else 2),
            text_style.size - (1 if outside else 0),
            text_style.color,
        )
        draw_inline_clock(font, chip, spec.segments, _clock_value(state, clock_id), scale=scale)
        if vertical:
            y -= chip_height + 4.0 * scale if outside else -(chip_height + 4.0 * scale)
        else:
...
from sincity.presentation.tags import (  # noqa: F401
    condition_labels,
    requirement_labels,
    action_corner_labels,
    action_starts_encounter,
